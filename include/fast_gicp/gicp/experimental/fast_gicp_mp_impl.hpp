...
template <typename PointSource, typename PointTarget>
void FastGICPMultiPoints<PointSource, PointTarget>::computeTransformation(PointCloudSource& output,
                                                                          const Matrix4& guess)
{
...
template <typename PointSource, typename PointTarget>
Eigen::VectorXf FastGICPMultiPoints<PointSource, PointTarget>::loss_ls(const Eigen::Matrix<float, 6, 1>& x,
...
bool FastGICPMultiPoints<PointSource, PointTarget>::calculate_covariances(
    const boost::shared_ptr<const pcl::PointCloud<PointT>>& cloud,
